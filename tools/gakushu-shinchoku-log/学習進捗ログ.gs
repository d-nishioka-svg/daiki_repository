...
function getNextScheduleList_(ss) {
  ...
  for (var r = 0; r < values.length; r++) {
    ...
    out.push({
      ...
      nextDate: formatScheduleValue_(row[2]),
      nextStart: formatScheduleValue_(row[3]),
      nextEnd: formatScheduleValue_(row[4]),
      recordedDate: formatScheduleValue_(row[5]),
      updatedAt: formatScheduleValue_(row[6])
    });
  }
