...
1. **カレンダー件名と企業名の突合 → 実データ検証済み、残る確認はPOODLE側のみ**
   ...
   - さらに、予定の `description` に必ず
     `<a href="https://tsr.race.co.jp/pdf.php?id=NNNNNNN">企業情報</a>` という
     形でTSR企業IDが埋め込まれていることを確認した
     （`extractTsrId` で抽出済み、`ConsultationEvent.tsrId`）。
   - **残る確認事項はPOODLE側のみ**：POODLEの案件検索がこのTSR IDで検索できるか。
     できるなら、企業名の表記ゆれ（略称・株式会社の有無等）の問題自体を
     ほぼ回避できる。できない場合のみ、企業名でのあいまい検索 or
     `config/company-mapping.json` による手動マッピングにフォールバックする。
...
4. **重複登録防止の判定方法**
   ...
   - 実データ確認: 同一企業が同日に複数の別イベント（受講者ごとに30分刻みで
     別枠）を持つケースが多数あった（例: ある企業で同日5件）。よって重複判定・
     実施報告の単位は「企業単位」ではなく**イベント（日付＋開始/終了時刻）単位**
     で行う設計で問題ない。
5. **受講者管理アプリ（フェーズ2）**
