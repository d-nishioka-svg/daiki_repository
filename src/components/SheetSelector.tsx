    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 font-medium whitespace-normal">データの自動保存先となるGoogleスプレッドシートを設定・自動作成します</p>
        </div>
      </div>
      ...
      {selectedSheet ? (
        <motion.div
          ...
          className="bg-emerald-50/50 border border-emerald-100 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-2xs"
        >
          ...
          <div className="flex items-center gap-2.5 shrink-0 flex-wrap">
            <a
              ...
              className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 text-xs font-bold uppercase tracking-wider text-emerald-700 bg-emerald-50 hover:bg-emerald-100/90 transition-colors rounded-lg border border-emerald-200 min-h-[40px] shadow-2xs"
            >
        ...
        <div>
          <div className="flex flex-col sm:flex-row gap-2 p-1 bg-slate-100 rounded-xl mb-4 border border-slate-200/50">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                activeTab === "create" ? "bg-white text-emerald-700 shadow-2xs" : "text-slate-500 hover:text-slate-800"
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                activeTab === "connect" ? "bg-white text-emerald-700 shadow-2xs" : "text-slate-500 hover:text-slate-800"
              }`}
            >
          ...
          {error && (
            <div className="p-3 bg-red-50 border border-red-100 rounded-xl text-xs font-semibold text-red-600 mb-4 whitespace-normal break-words">
              {error}
            </div>
          )}
          ...
          {activeTab === "create" ? (
            <form onSubmit={handleCreate} className="space-y-3">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">作成するGoogleスプレッドシート名 / Sheet Title</label>
                <input
                  type="text"
                  value={newTitle}
                  onChange={(e) => setNewTitle(e.target.value)}
                  placeholder="e.g. 衣料品スキャン記録 / Garments Records"
                  className="w-full text-sm font-semibold px-3.5 py-3 border border-slate-200 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl text-slate-800 transition-all bg-slate-50/20 min-h-[48px]"
                  disabled={loading}
                />
              </div>
              <button
                ...
                className="w-full py-3 px-4 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 disabled:bg-slate-100 disabled:text-slate-400 text-white font-bold uppercase tracking-wider text-xs md:text-sm rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 shadow-xs min-h-[48px]"
              >
            ...
            <form onSubmit={handleConnect} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs md:text-sm font-mono px-3.5 py-3 border border-slate-200 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl text-slate-800 transition-all bg-slate-50/20 min-h-[48px]"
                  disabled={loading}
                />
              </div>
              <button
                ...
                className="w-full py-3 px-4 bg-slate-800 hover:bg-slate-900 active:bg-slate-950 disabled:bg-slate-100 disabled:text-slate-400 text-white font-bold uppercase tracking-wider text-xs md:text-sm rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 shadow-xs min-h-[48px]"
              >
